...
function setupLoginForm() {
  ...
  const DEMO_EMAIL = 'demo@cognitivecare.com';
  const DEMO_PASSWORD = 'caregiver123';
  ...
  function getAccount() {
    try {
      ...
      return account && typeof account.email === 'string' && typeof account.password === 'string'
        ? account
        : null;
    } catch (err) {
      console.warn('Invalid saved caregiver account; resetting it.', err);
      localStorage.removeItem(CAREGIVER_ACCOUNT_KEY);
      return null;
    }
  }
  ...
  // Restore a remembered login on this device.
  try {
    const remembered = localStorage.getItem('cognitive_care_remembered_login');
    if (remembered && emailInput) emailInput.value = remembered;
    if (remembered && rememberMe) rememberMe.checked = true;
  } catch (err) {
    console.warn('Could not restore remembered login.', err);
  }

  // Dedicated account-creation flow. The main button opens a real registration form.
  const createModal = document.getElementById('create-account-modal');
  const createForm = document.getElementById('create-account-form');
  const createClose = document.getElementById('create-account-close');
  const createCancel = document.getElementById('create-account-cancel');
  const createName = document.getElementById('create-name');
  const createEmail = document.getElementById('create-email');
  const createPassword = document.getElementById('create-password');
  const createPasswordConfirm = document.getElementById('create-password-confirm');
  const createError = document.getElementById('create-account-error');

  const closeCreateModal = () => {
    if (createModal) createModal.classList.add('hidden');
    if (createModal) createModal.classList.remove('flex');
  ...
  const openCreateModal = () => {
    ...
    if (createEmail && emailInput && emailInput.value.trim()) createEmail.value = emailInput.value.trim();
    if (createPassword && passwordInput && passwordInput.value) createPassword.value = passwordInput.value;
    setTimeout(() => createName?.focus(), 50);
  };

  createAccountBtn?.addEventListener('click', openCreateModal);
  ...
  createForm?.addEventListener('submit', (e) => {
    ...
    const fail = (msg) => { if (createError) { createError.textContent = msg; createError.classList.remove('hidden'); } };
    if (!name || !email || !password || !confirm) return fail('Please fill in every field.');
    if (password.length < 4) return fail('Password should be at least 4 characters.');
    if (password !== confirm) return fail('Passwords do not match.');

    try {
      localStorage.setItem(CAREGIVER_ACCOUNT_KEY, JSON.stringify({ name, email, password, createdAt: new Date().toISOString() }));
      localStorage.setItem('cognitive_care_remembered_login', email);
      sessionStorage.setItem('cognitive_care_logged_in', 'true');
    } catch (err) {
      console.error('Error creating caregiver account:', err);
      return fail('Your browser blocked site storage. Please allow storage for this site and try again.');
    }
  ...
  form.addEventListener('submit', (e) => {
    ...
    const validAgainstSavedAccount = account &&
      enteredEmail.toLowerCase() === account.email.trim().toLowerCase() &&
      enteredPassword === account.password;
    const validDemoLogin = !account &&
      enteredEmail.toLowerCase() === DEMO_EMAIL &&
      enteredPassword === DEMO_PASSWORD;

    if (!validAgainstSavedAccount && !validDemoLogin) {
      showError(account
        ? 'Incorrect email/phone or password.'
        : 'No account found. Use demo@cognitivecare.com / caregiver123 or create an account.');
      return;
    }

    // If the demo login was used, create a usable local account automatically.
    if (!account && validDemoLogin) {
      try {
        localStorage.setItem(CAREGIVER_ACCOUNT_KEY, JSON.stringify({ email: DEMO_EMAIL, password: DEMO_PASSWORD }));
      } catch (err) {
        console.warn('Could not save demo account.', err);
      }
    }

    try {
      if (rememberMe && rememberMe.checked) {
        localStorage.setItem('cognitive_care_remembered_login', enteredEmail);
      } else {
        localStorage.removeItem('cognitive_care_remembered_login');
      }
      sessionStorage.setItem('cognitive_care_logged_in', 'true');
    } catch (err) {
      console.warn('Could not save login session.', err);
    }
  ...
  if (forgotLink) {
    forgotLink.addEventListener('click', (e) => {
      ...
      const account = getAccount();
      if (account) {
        alert('For this demo, your current caregiver password is stored only in this browser. Use Settings to change it after logging in.');
      } else {
        alert(`Demo login:
Email: ${DEMO_EMAIL}
...
// 7. Stable live clock + account menu + progress filters
function setupClock() {
...
function setupTopProfileMenu() {
  const roots = document.querySelectorAll('[data-profile-menu]');
  ...
  roots.forEach(root => {
    ...
    const sync = () => {
      let account = null;
      try {
        const raw = localStorage.getItem(CAREGIVER_ACCOUNT_KEY);
        if (raw) account = JSON.parse(raw);
      } catch (_) {}
      const loggedIn = sessionStorage.getItem('cognitive_care_logged_in') === 'true';
      const emailEl = root.querySelector('.profile-menu-email');
      const loginBtn = root.querySelector('.profile-login');
      const logoutBtn = root.querySelector('.profile-logout');
      if (emailEl) emailEl.textContent = loggedIn && account?.email ? account.email : 'Not signed in';
    ...
    btn.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.profile-menu-btn').forEach(b => b.setAttribute('aria-expanded', 'false'));
      if (!open) {
    ...
    root.querySelector('.profile-login')?.addEventListener('click', () => { window.location.href = 'login.html'; });
    root.querySelector('.profile-logout')?.addEventListener('click', () => {
      sessionStorage.removeItem('cognitive_care_logged_in');
      localStorage.removeItem('cognitive_care_remembered_login');
      window.location.href = 'login.html';
    });
    document.addEventListener('click', (e) => { if (!root.contains(e.target)) { menu.classList.add('hidden'); btn.setAttribute('aria-expanded', 'false'); } });
    sync();
  });
}

function setupProgressPeriodFilter() {
  const buttons = document.querySelectorAll('[data-progress-period]');
  if (!buttons.length) return;
  let active = 'week';

  const parseDate = (game) => {
    if (game?.timestamp) return new Date(game.timestamp);
    return null;
  };

  const getFilteredGames = (state) => {
    const games = Array.isArray(state.recentGames) ? state.recentGames : [];
    if (active === 'all') return games;
    const now = new Date();
    const cutoff = new Date(now);
    cutoff.setHours(0,0,0,0);
    cutoff.setDate(cutoff.getDate() - (active === 'week' ? 6 : 29));
  ...
  const paint = () => {
    const state = getCognitiveState();
    const games = getFilteredGames(state);
    const count = games.length;
    const accuracy = count ? Math.round(games.reduce((sum,g)=>sum + Number(g.score || 0),0) / count) : 0;
    const actEl = document.getElementById('stat-activities');
    const accEl = document.getElementById('stat-accuracy');
    if (actEl) actEl.textContent = count;
    if (accEl) accEl.textContent = `${accuracy}%`;
    const list = document.getElementById('recent-games-list');
    if (list) {
      if (!count) {
        list.innerHTML = '<div class="p-6 text-center text-slate-400 text-xs bg-slate-50 rounded-2xl border border-dashed border-slate-200">No games in this period yet.</div>';
      } else {
        list.innerHTML = games.map(game => `<div class="flex items-center justify-between p-3.5 rounded-2xl bg-slate-50 border border-slate-100"><div class="flex items-center gap-3"><div class="w-9 h-9 rounded-xl ${game.bg || 'bg-teal-100'} ${game.text || 'text-teal-700'} flex items-center justify-center"><i data-lucide="${game.icon || 'gamepad-2'}" class="w-4 h-4"></i></div><div><div class="font-bold text-slate-900 text-xs">${game.name || 'Game'}</div><div class="text-[11px] text-slate-400">${game.timestamp ? new Date(game.timestamp).toLocaleDateString('en-US',{month:'short',day:'numeric'}) : (game.time || '')}</div></div></div><div class="text-xs font-bold text-slate-900">${Number(game.score || 0)}% <span class="text-[10px] font-normal text-slate-400">Score</span></div></div>`).join('');
        if (window.lucide) window.lucide.createIcons();
      }
    }
    buttons.forEach(b => {
      const on = b.getAttribute('data-progress-period') === active;
      b.className = on ? 'bg-brand-navy text-white px-3.5 py-1.5 rounded-xl shadow-xs' : 'text-slate-600 hover:text-slate-900 px-3.5 py-1.5 transition-colors';
    });
  };

  buttons.forEach(b => b.addEventListener('click', () => { active = b.getAttribute('data-progress-period') || 'week'; paint(); }));
  paint();
}
...
function recordCompletedActivity(gameName, category = 'memory', score = 98) {
  ...
  // Keep a useful history for the progress filters.
  if (state.recentGames.length > 100) {
    state.recentGames.length = 100;
  }
...
// 7. Caregiver-managed Daily Routines & Medication Plan
const CARE_ROUTINES_KEY = 'cognitive_care_daily_routines';
const CARE_MEDS_KEY = 'cognitive_care_medications';
const DEFAULT_ROUTINES = [
  {id:'r1', title:'Morning Walk', time:'8:00 AM', note:'15 minutes', done:true},
  {id:'r2', title:'Healthy Breakfast', time:'8:30 AM', note:'Oatmeal & fruit', done:true},
  {id:'r3', title:'Afternoon Rest', time:'2:00 PM', note:'Quiet relaxing time', done:false}
];
const DEFAULT_MEDS = [
  {id:'m1', title:'Morning Pill (Donepezil)', dose:'1 Tablet (5mg)', time:'8:00 AM', done:true},
  {id:'m2', title:'Afternoon Vitamin & Omega-3', dose:'2 Capsules', time:'1:00 PM', done:false},
  {id:'m3', title:'Evening Medication (Memantine)', dose:'1 Tablet (10mg)', time:'8:00 PM', done:false}
];
function readPlan(key, fallback){
  try { const raw=localStorage.getItem(key); return raw ? JSON.parse(raw) : fallback.map(x=>({...x})); }
  catch(e){ return fallback.map(x=>({...x})); }
}
function writePlan(key,val){ localStorage.setItem(key, JSON.stringify(val)); }
function escapePlanText(v){ const d=document.createElement('div'); d.textContent=v??''; return d.innerHTML; }
function renderCarePlan(){
  const rList=document.getElementById('routine-plan-list'); const mList=document.getElementById('med-plan-list');
  if(!rList && !mList) return;
  const routines=readPlan(CARE_ROUTINES_KEY,DEFAULT_ROUTINES); const meds=readPlan(CARE_MEDS_KEY,DEFAULT_MEDS);
  if(rList){
    rList.innerHTML=routines.length ? routines.map(r=>`<div class="flex items-center justify-between gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full ${r.done?'bg-teal-700 text-white':'bg-slate-200 text-slate-600'} flex items-center justify-center"><i data-lucide="${r.done?'check':'clock'}" class="w-4 h-4"></i></div><div><div class="font-bold text-slate-800 text-sm">${escapePlanText(r.title)}</div><div class="text-xs text-slate-500">${escapePlanText(r.time)} • ${escapePlanText(r.note||'')}</div></div></div><button data-routine-id="${escapePlanText(r.id)}" class="routine-done-btn ${r.done?'text-teal-700':'bg-brand-navy text-white'} ${r.done?'p-2':'px-3 py-2'} rounded-xl text-xs font-semibold">${r.done?'✓':'Mark Done'}</button></div>`).join('') : `<div class="text-sm text-slate-400 bg-slate-50 rounded-2xl p-5 text-center">No routines added yet.</div>`;
  }
  if(mList){
    const pending=meds.filter(m=>!m.done).length; const badge=document.getElementById('med-count'); if(badge) badge.textContent=`${pending} Pending`;
    mList.innerHTML=meds.length ? meds.map(m=>`<div class="flex items-center justify-between gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full ${m.done?'bg-teal-700 text-white':'bg-slate-200 text-slate-600'} flex items-center justify-center"><i data-lucide="${m.done?'check':'pill'}" class="w-4 h-4"></i></div><div><div class="font-bold text-slate-800 text-sm">${escapePlanText(m.title)}</div><div class="text-xs text-slate-500">${escapePlanText(m.dose||'')} • ${escapePlanText(m.time||'')}${m.done?' • Taken':''}</div></div></div><button data-med-id="${escapePlanText(m.id)}" class="med-done-btn ${m.done?'text-teal-700':'bg-brand-navy text-white'} ${m.done?'p-2':'px-3 py-2'} rounded-xl text-xs font-semibold">${m.done?'✓':'Take Now'}</button></div>`).join('') : `<div class="text-sm text-slate-400 bg-slate-50 rounded-2xl p-5 text-center">No medications added yet.</div>`;
  }
  const rc=document.getElementById('routine-count'); const done=routines.filter(r=>r.done).length; if(rc) rc.textContent=`${done} of ${routines.length} Done`;
  if(window.lucide) window.lucide.createIcons();
}
function setupCarePlanEditor(){
  const open1=document.getElementById('edit-daily-plan-btn'), open2=document.getElementById('edit-daily-plan-btn-2'), back=document.getElementById('care-plan-backdrop');
  if(!back || (!open1 && !open2)) { renderCarePlan(); return; }
  const passScreen=document.getElementById('care-plan-password-screen'), editor=document.getElementById('care-plan-editor');
  const pass=document.getElementById('care-plan-password'), err=document.getElementById('care-plan-password-error');
  const rEd=document.getElementById('routine-editor-list'), mEd=document.getElementById('med-editor-list');
  function rowR(r){ return `<div class="grid grid-cols-[1.3fr_.7fr_1fr_auto] gap-2 items-center" data-rrow="${r.id}"><input class="r-title bg-slate-100 rounded-xl px-3 py-2 text-sm" value="${escapePlanText(r.title)}" placeholder="Routine"><input class="r-time bg-slate-100 rounded-xl px-3 py-2 text-sm" value="${escapePlanText(r.time||'')}" placeholder="Time"><input class="r-note bg-slate-100 rounded-xl px-3 py-2 text-sm" value="${escapePlanText(r.note||'')}" placeholder="Note"><button class="remove-r text-rose-500 p-2" type="button">✕</button></div>`; }
  function rowM(m){ return `<div class="grid grid-cols-[1.3fr_.8fr_.7fr_auto] gap-2 items-center" data-mrow="${m.id}"><input class="m-title bg-slate-100 rounded-xl px-3 py-2 text-sm" value="${escapePlanText(m.title)}" placeholder="Medication"><input class="m-dose bg-slate-100 rounded-xl px-3 py-2 text-sm" value="${escapePlanText(m.dose||'')}" placeholder="Dose"><input class="m-time bg-slate-100 rounded-xl px-3 py-2 text-sm" value="${escapePlanText(m.time||'')}" placeholder="Time"><button class="remove-m text-rose-500 p-2" type="button">✕</button></div>`; }
  function loadEditor(){ rEd.innerHTML=readPlan(CARE_ROUTINES_KEY,DEFAULT_ROUTINES).map(rowR).join(''); mEd.innerHTML=readPlan(CARE_MEDS_KEY,DEFAULT_MEDS).map(rowM).join(''); }
  function open(){ back.classList.remove('hidden'); passScreen.classList.remove('hidden'); editor.classList.add('hidden'); pass.value=''; err.classList.add('hidden'); }
  [open1,open2].filter(Boolean).forEach(b=>b.addEventListener('click',open));
  document.getElementById('care-plan-close-btn').addEventListener('click',()=>back.classList.add('hidden'));
  document.getElementById('care-plan-cancel-btn').addEventListener('click',()=>back.classList.add('hidden'));
  document.getElementById('care-plan-unlock-btn').addEventListener('click',()=>{ if(pass.value===getCaregiverEditPassword()){ passScreen.classList.add('hidden'); editor.classList.remove('hidden'); loadEditor(); } else { err.classList.remove('hidden'); } });
  document.getElementById('add-routine-row').addEventListener('click',()=>{ rEd.insertAdjacentHTML('beforeend',rowR({id:'r'+Date.now(),title:'New routine',time:'',note:''})); });
  document.getElementById('add-med-row').addEventListener('click',()=>{ mEd.insertAdjacentHTML('beforeend',rowM({id:'m'+Date.now(),title:'New medication',dose:'',time:''})); });
  rEd.addEventListener('click',e=>{if(e.target.closest('.remove-r')) e.target.closest('[data-rrow]')?.remove();});
  mEd.addEventListener('click',e=>{if(e.target.closest('.remove-m')) e.target.closest('[data-mrow]')?.remove();});
  document.getElementById('care-plan-save-btn').addEventListener('click',()=>{
    const routines=[...rEd.querySelectorAll('[data-rrow]')].map(row=>({id:row.dataset.rrow,title:row.querySelector('.r-title').value.trim(),time:row.querySelector('.r-time').value.trim(),note:row.querySelector('.r-note').value.trim(),done:false})).filter(x=>x.title);
    const meds=[...mEd.querySelectorAll('[data-mrow]')].map(row=>({id:row.dataset.mrow,title:row.querySelector('.m-title').value.trim(),dose:row.querySelector('.m-dose').value.trim(),time:row.querySelector('.m-time').value.trim(),done:false})).filter(x=>x.title);
    writePlan(CARE_ROUTINES_KEY,routines); writePlan(CARE_MEDS_KEY,meds); renderCarePlan(); back.classList.add('hidden'); speak('Daily care plan updated.');
  });
  const rl=document.getElementById('routine-plan-list'); const ml=document.getElementById('med-plan-list');
  if(rl) rl.addEventListener('click',e=>{ const b=e.target.closest('.routine-done-btn'); if(!b)return; const id=b.dataset.routineId; const arr=readPlan(CARE_ROUTINES_KEY,DEFAULT_ROUTINES); const item=arr.find(x=>x.id===id); if(item){item.done=true;writePlan(CARE_ROUTINES_KEY,arr);renderCarePlan();} });
  if(ml) ml.addEventListener('click',e=>{ const b=e.target.closest('.med-done-btn'); if(!b)return; const id=b.dataset.medId; const arr=readPlan(CARE_MEDS_KEY,DEFAULT_MEDS); const item=arr.find(x=>x.id===id); if(item){item.done=true;writePlan(CARE_MEDS_KEY,arr);renderCarePlan();} });
  renderCarePlan();
}
const __oldDOMContentLoaded = null;
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', setupCarePlanEditor);
else setupCarePlanEditor();
